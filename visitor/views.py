...
from general.init_cache import (
    get_all_user,
    get_specific_user_articles_by_username,
    get_specific_user_favorite_articles_by_username,
...
def get_specific_user_favorite_articles(request):
    if request.method == "POST":
        try:
            ...
            return JsonResponse({"code": 401, "msg": "failed with wrong visitor or invalid params value"})
        except TypeError:
            return JsonResponse({"code": 401, "msg": "failed with wrong visitor or invalid params"})
            ...
            matched_favorite_articles = [
                {
                    ...
                    "favorite_article": {
                        ...
                        "article_user": favorite.correlation_article.visitor.username,
                    },
                    "favorite_user": {
                        "user_id": favorite.visitor.id,
                        "user_username": favorite.visitor.username,
                        "user_email": favorite.visitor.email,
                    },
...
def get_specific_user_articles(request):
    if request.method == "POST":
        ...
        try:
            ...
            return JsonResponse({"code": 402, "msg": "failed with wrong visitor or invalid params"})
            ...
            articles = [
                {
                    ...
                    if article.correlation_software
                    else None,
                    "article_user": {
                        "user_id": article.visitor.id,
                        "user_username": article.visitor.username,
                        "user_email": article.visitor.email,
                    },
                }
            ...
            software = [
                {
                    ...
                    "software_user": {
                        "user_id": soft_ware.visitor.id,
                        "user_username": soft_ware.visitor.username,
                        "user_email": soft_ware.visitor.email,
                    },
                }
            ...
            matched_software = [
                {
                    ...
                    "software_user": {
                        "user_id": software.visitor.id,
                        "user_username": software.visitor.username,
                        "user_email": software.visitor.email,
                    },
                }
...
def login(request):
    if request.method == "POST":
        ...
        if username and password:
            ...
            user = [
                x for x in get_all_user() if x.username == username and check_password(password, x.django_user.password)
            ]
            if len(user) > 1:
                return JsonResponse({"code": 401, "msg": "failed with wrong visitor or invalid params"}, status=401)
            if len(user) <= 0:
                return JsonResponse(
                    {"code": 404, "msg": "failed with no matched visitor or wrong password"}, status=401
                )
            if len(user) == 1:
...
def register(request):
    if request.method == "POST":
        ...
        if username and password and email and repeat_password:
            if len(Visitor.objects.filter(username=username)) > 0:
                return JsonResponse({"code": 401, "msg": "failed with existed visitor"})
            ...
            created_user = Visitor.objects.get(username=username)
            head_icon_path = storage_uploaded_image(request.FILES.get("head_icon"), "visitor/head_icon/")
...
def update_user(request):
    if request.method == "PUT":
        ...
        try:
            ...
            if head_icon:
                head_icon_path = storage_uploaded_image(head_icon, "visitor/head_icon/")
                user.head_icon = head_icon_path

            if user_center_cover:
                user_center_cover_path = storage_uploaded_image(user_center_cover, "visitor/user_center_cover/")
            ...
            return JsonResponse({"code": 404, "msg": "visitor not found"}, status=404)
        except Exception as e:
            return JsonResponse({"code": 401, "msg": str(e)}, status=401)
    return JsonResponse({"code": 400, "msg": "failed with wrong request action"}, status=400)
...
def cancel_user(request):
    if request.method == "DELETE":
        try:
            ...
            return JsonResponse({"code": 400, "msg": "Invalid JSON"}, status=400)
        if user_id:
            if len(Visitor.objects.filter(id=user_id)) == 1:
                try:
                    # visitor = Visitor.objects.get(id=user_id)
                    if request.session.get("logon_user"):
                        del request.session["logon_user"]
                        django_logout(request)
                    user = DjangoUser.objects.get(id=Visitor.objects.get(id=user_id).django_user_id)
                    user.delete()
                except Exception as e:
                    return JsonResponse({"code": 401, "msg": str(e)}, status=401)
            return JsonResponse({"code": 200, "msg": "success"}, status=200)
...
def user_detail_page(request):
    if request.method == "GET":
        ...
        try:
            ...
            return render(
                request, "404.html", {"code": 404, "err": "failed with wrong visitor or invalid params value"}
            )
        except TypeError:
            return render(request, "404.html", {"code": 402, "err": "failed with wrong visitor or invalid params"})
        except Exception as e:
            return render(request, "404.html", {"code": 403, "err": str(e)})
        if user_id:
            user = [x for x in get_all_user() if x.id == user_id]
            if len(user) == 1:
                published_articles_count = len(user[0].article_set.all())
                published_software_count = len([x for x in get_specific_user_software_by_username(user[0].username)])
                published_comments_count = len(user[0].comment_set.all())
                recent_browsing_count = len(user[0].recentbrowsing_set.all())
                current_software_set = [x for x in get_specific_user_software_by_username(user[0].username)]
                return render(
                    request,
                    "user_details.html",
                    {
            ...
            return render(request, "404.html", {"code": 404, "err": "request succeed, failed with no matched visitor"})
        return render(request, "404.html", {"code": 400, "err": "failed with wrong request action"})
    return render(request, "404.html", {"code": 400, "err": "failed with wrong request action"})
