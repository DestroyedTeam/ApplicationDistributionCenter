...
from general.common_compute import get_software_hot_degree as g_h_s
from general.init_cache import get_all_articles as g_a_as
from general.init_cache import get_all_software as g_a_s
from general.init_cache import get_all_user as g_a_u
...
def index(request):
    # 获取网站相关指标
    indicators = {}
    recent_appended_user = FrontEndUser.objects.filter(django_user__date_joined__gte=datetime.now() - timedelta(days=7))
    ...
    indicators["all_comments"] = {"name": "总评论数", "value": len([comment for comment in get_comments()])}
    indicators["all_thumbs"] = {
        "name": "总点赞数",
        "value": sum([i.thumbs_volume for i in g_a_as()]) + sum([i.thumbs_volume for i in g_a_s()]),
    }
    # 创建一个图表对象
    bar = Bar()
    ...
    return render(request, "index.html", {"chart1": bar.dump_options_with_quotes(), "indicators": indicators})
